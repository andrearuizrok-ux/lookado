...
function editBusinessIdentity(){const b=businessProfileData.business;openModal("Informazioni attività",`<form id="bizIdentityForm" class="form-grid"><div class="field full"><label>Nome attività</label><input name="name" value="${escapeHtml(b.name||"")}" required></div><div class="field"><label>Modalità di lavoro</label><select name="work_mode" onchange="toggleEditWorkMode()">${Object.entries(WORK_MODE_META).map(([id,x])=>`<option value="${id}" ${b.work_mode===id?"selected":""}>${x.label}</option>`).join("")}</select></div><div class="field"><label>Telefono</label><input name="phone" value="${escapeHtml(b.phone||"")}"></div><div class="field"><label>Paese</label><input name="country" value="${escapeHtml(b.country||"")}"></div><div class="field"><label>Codice Paese</label><input name="country_code" maxlength="2" value="${escapeHtml(b.country_code||"")}"></div><div class="field"><label>Città / zona</label><input name="city" value="${escapeHtml(b.city||"")}"></div><div class="field" id="editAddressWrap"><label>Indirizzo</label><input name="address" value="${escapeHtml(b.address||"")}"></div><div class="field"><label>Valuta</label><input name="currency" maxlength="3" value="${escapeHtml(b.currency||"EUR")}"></div><div class="field"><label>Timezone</label><input name="timezone" value="${escapeHtml(b.timezone||"Europe/Rome")}"></div><div class="field full" id="editPrivacyWrap"><label class="privacy-choice"><input type="checkbox" name="public_address" ${b.public_address!==false?"checked":""}><span><strong>Mostra indirizzo esatto pubblicamente</strong><small>Disattivalo per proteggere la privacy di un home studio.</small></span></label></div><div class="field full" id="editRadiusWrap"><label>Raggio servizio a domicilio (km)</label><input type="number" min="1" max="250" name="service_radius_km" value="${b.service_radius_km||10}"></div><div class="field full"><label>Descrizione</label><textarea name="description" rows="5">${escapeHtml(b.description||"")}</textarea></div></form>`,`<button class="btn ghost" onclick="closeModal()">Annulla</button><button class="btn primary" onclick="saveBusinessIdentity()">Salva</button>`);setTimeout(toggleEditWorkMode,0)}
function toggleEditWorkMode(){const f=document.getElementById("bizIdentityForm");if(!f)return;const mode=f.elements.work_mode.value;document.getElementById("editPrivacyWrap")?.classList.toggle("hidden",mode==="mobile");document.getElementById("editRadiusWrap")?.classList.toggle("hidden",!(mode==="mobile"||mode==="hybrid"))}
async function saveBusinessIdentity(){const f=document.getElementById("bizIdentityForm");if(!f.reportValidity())return;const fd=new FormData(f),mode=fd.get("work_mode");const payload={name:fd.get("name")||null,phone:fd.get("phone")||null,country:fd.get("country")||null,country_code:String(fd.get("country_code")||"").trim().toUpperCase()||null,city:fd.get("city")||null,address:fd.get("address")||null,currency:String(fd.get("currency")||"EUR").toUpperCase(),timezone:fd.get("timezone")||"Europe/Rome",description:fd.get("description")||null,work_mode:mode,public_address:mode==="mobile"?false:fd.get("public_address")==="on",service_radius_km:(mode==="mobile"||mode==="hybrid")?Number(fd.get("service_radius_km")||10):null};const {error}=await db.from("businesses").update(payload).eq("id",state.proBusinessId);if(error){toast(error.message);return}closeModal();await loadBusinessProfilePro();toast("Profilo aggiornato")}
...
const WORK_MODE_META={
 venue:{label:"Locale / studio",desc:"Ricevi i clienti in un salone, studio o locale.",icon:"⌂"},
 home_studio:{label:"Ricevo a casa",desc:"Home studio: puoi nascondere l'indirizzo esatto fino alla conferma.",icon:"⌾"},
 mobile:{label:"Vado dal cliente",desc:"Lavori a domicilio nell'area che scegli.",icon:"↗"},
 hybrid:{label:"Entrambe le modalità",desc:"Ricevi in una sede e offri anche servizi a domicilio.",icon:"⇄"}
};
function workModeLabel(mode){return WORK_MODE_META[mode]?.label||WORK_MODE_META.venue.label}
function workModePublicLocation(b){
 const city=[b.city,b.country].filter(Boolean).join(", ");
 if(b.work_mode==="mobile") return `${city||"Area da definire"} · Servizio a domicilio`;
 if(b.work_mode==="home_studio" && b.public_address===false) return `${city||"Zona da definire"} · Home studio`;
 if(b.work_mode==="hybrid" && b.public_address===false) return `${city||"Zona da definire"} · Studio + domicilio`;
 return [b.address,city].filter(Boolean).join(", ")||"Posizione da aggiungere";
}
function updateWorkModeFields(){
 const form=document.getElementById("createBusinessForm"); if(!form)return;
 const mode=form.querySelector('input[name="work_mode"]:checked')?.value||"venue";
 const address=form.querySelector('[name="address"]'), radius=form.querySelector('[name="service_radius_km"]');
 const privacy=document.getElementById("addressPrivacyWrap"), radiusWrap=document.getElementById("serviceRadiusWrap");
 if(address){
   address.required=(mode==="venue"||mode==="hybrid");
   address.placeholder=mode==="home_studio"?"Indirizzo home studio (facoltativo)":"Via e numero civico";
 }
 if(privacy) privacy.classList.toggle("hidden",mode==="mobile");
 if(radiusWrap) radiusWrap.classList.toggle("hidden",!(mode==="mobile"||mode==="hybrid"));
 if(radius && (mode==="mobile"||mode==="hybrid") && !radius.value) radius.value="10";
}
...
function createBusinessView(){
  ...
  setTimeout(updateWorkModeFields,0);
  return `<div class="shell onboarding-shell">
    ...
    <form id="createBusinessForm" onsubmit="createBusinessLookado(event)">
        ...
        <div class="setup-grid">
          <div class="setup-main panel"><div class="eyebrow">01 · IDENTITÀ</div><h2>Raccontaci chi siete</h2>
            <div class="form-grid">
              ...
              <div class="field full"><label>Come lavori? *</label><div class="work-mode-grid">${Object.entries(WORK_MODE_META).map(([id,m])=>`<label class="work-mode-card"><input type="radio" name="work_mode" value="${id}" ${id==="venue"?"checked":""} onchange="updateWorkModeFields()"><span class="work-mode-icon">${m.icon}</span><strong>${m.label}</strong><small>${m.desc}</small></label>`).join("")}</div></div>
      ...
      <section class="setup-page hidden" data-setuppage="1"><div class="panel"><div class="eyebrow">02 · CONTATTI & POSIZIONE</div><h2>Fatti trovare facilmente</h2><div class="form-grid">
        ...
        <div class="field"><label>Paese *</label><input name="country" value="${escapeHtml(userPlace?.country||"")}" placeholder="Es. Italia" required></div>
        <div class="field"><label>Codice Paese</label><input name="country_code" value="${escapeHtml(userPlace?.countryCode||"")}" maxlength="2" placeholder="IT"></div>
        <div class="field"><label>Città / zona *</label><input name="city" value="${escapeHtml(userPlace?.city||"")}" placeholder="Es. Milano" required></div>
        <div class="field"><label>Indirizzo</label><input name="address" placeholder="Via e numero civico"></div>
        <div class="field full" id="addressPrivacyWrap"><label class="privacy-choice"><input type="checkbox" name="public_address" value="1" checked><span><strong>Mostra l'indirizzo esatto nel profilo pubblico</strong><small>Per un home studio puoi disattivarlo: mostreremo solo città/zona e l'indirizzo potrà essere comunicato dopo la conferma.</small></span></label></div>
        <div class="field full hidden" id="serviceRadiusWrap"><label>Area servizio a domicilio</label><div class="radius-row"><input name="service_radius_km" type="number" min="1" max="250" value="10"><span>km dalla tua zona</span></div></div>
        <div class="field"><label>Valuta</label><select name="currency"><option>EUR</option><option>GBP</option><option>USD</option><option>CAD</option><option>BRL</option><option>MXN</option><option>VES</option></select></div>
        <div class="field"><label>Fuso orario</label><input name="timezone" value="${escapeHtml(Intl.DateTimeFormat().resolvedOptions().timeZone||"Europe/Rome")}"></div>
        <div class="field full"><button type="button" class="btn secondary small" onclick="fillBusinessLocationFromDevice()">⌖ Usa la mia posizione</button><div class="meta">Compiliamo città e Paese dalla posizione del dispositivo. Puoi sempre correggerli.</div></div>
...
async function fillBusinessLocationFromDevice(){
 const form=document.getElementById("createBusinessForm"); if(!form)return;
 if(!navigator.geolocation){showBusinessError("La geolocalizzazione non è disponibile su questo dispositivo.");return}
 navigator.geolocation.getCurrentPosition(async pos=>{
   const place=await reverseGeocode(pos.coords.latitude,pos.coords.longitude);
   if(place){
     form.elements.city.value=place.city||"";
     form.elements.country.value=place.country||"";
     form.elements.country_code.value=place.countryCode||"";
   }
   form.dataset.latitude=String(pos.coords.latitude);
   form.dataset.longitude=String(pos.coords.longitude);
   toast(place?[place.city,place.country].filter(Boolean).join(", "):"Posizione rilevata");
 },()=>showBusinessError("Non è stato possibile leggere la posizione. Puoi inserire città e Paese manualmente."),
 {enableHighAccuracy:false,timeout:8000,maximumAge:300000});
}
...
async function createBusinessLookado(e){
 ...
 try{
   ...
   if(error)throw error;
   const mode=fd.get("work_mode")||"venue";
   const publicAddress=mode==="mobile"?false:fd.get("public_address")==="1";
   const extraProfile={
     work_mode:mode,
     public_address:publicAddress,
     service_radius_km:(mode==="mobile"||mode==="hybrid")?Number(fd.get("service_radius_km")||10):null,
     country:fd.get("country")||null,
     country_code:String(fd.get("country_code")||"").trim().toUpperCase()||null,
     currency:fd.get("currency")||"EUR",
     timezone:fd.get("timezone")||Intl.DateTimeFormat().resolvedOptions().timeZone||"Europe/Rome",
     latitude:form.dataset.latitude?Number(form.dataset.latitude):null,
     longitude:form.dataset.longitude?Number(form.dataset.longitude):null
   };
   const {error:profileError}=await db.from("businesses").update(extraProfile).eq("id",id);
   ...
   const agendaMode=fd.get("agenda_mode");
   if(agendaMode==="google"||agendaMode==="csv") await db.from("calendar_integrations").upsert({business_id:id,user_id:authUser.id,provider:agendaMode,status:"disconnected"});
